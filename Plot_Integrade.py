...
if __name__ == '__main__':
    ...
    for file_name in File_Names:
        ...
        for timestamp in dict_result.keys():
            grade = dict_result[timestamp]
            dict_all = add_to_dict(dict_all, timestamp, grade)
        
    #---/Sub settings/---
    Grades = np.zeros_like(Xs)
    Lst_name = file_name.split('_')
    #--------------------------------------------------------------------------------------------------
    reflectance = '8%' # <<<<<<<<============================<<<<<<<< CHANGE REFLECTANCE HERE <<<<<<<<
    #--------------------------------------------------------------------------------------------------
            ...
            if doy:
                ...
                for hh in range(hh_start, hh_end):
                    ...
                    for mimi in range(60):
                        ...
                        Grades[miod, doy] = get_grade(timestamp, dict_all)
    
    #-/Initiate the figure/-
    fig, ax = plt.subplots(layout = 'compressed', figsize = (24, 30))
    font01 = {'color':'black', 'size':20}
    font02 = {'color':'black', 'size':30}
    txt_title = 'Annual Glare Rates of all Locations with Glass Reflectance = ' + reflectance
    plt.title(txt_title, fontdict = font02)
    ...
    #plt.show()
    figure_name = 'Locates_ALL_R' + reflectance + '.png'
